...
class Empresa extends Conectar
{
    ...
    public function get_id_por_nombre($emp_nom) {
        $conectar = parent::conexion();
        $sql = "SELECT emp_id FROM td_empresa WHERE UPPER(emp_nom) = UPPER(?) AND est = 1 LIMIT 1";
        $sql = $conectar->prepare($sql);
        $sql->bindValue(1, trim($emp_nom));
        $sql->execute();
        $resultado = $sql->fetch(PDO::FETCH_ASSOC);
        return $resultado ? $resultado['emp_id'] : null;
    }
}
