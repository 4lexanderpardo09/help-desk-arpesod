...
switch ($_GET["op"]) {
    ...
    case "listar":
        ...
        foreach ($datos as $row) {
            ...
            // CAMBIADO: Se usan las nuevas columnas con los nombres concatenados (Cargos y Perfiles)
            $creadores_texto = "";
            if (!empty($row["creadores_cargos"])) {
                $creadores_texto .= "Cargos: " . $row["creadores_cargos"];
            }
            if (!empty($row["creadores_perfiles"])) {
                if (!empty($creadores_texto)) $creadores_texto .= "<br>";
                $creadores_texto .= "Perfiles: " . $row["creadores_perfiles"];
            }
            $sub_array[] = !empty($creadores_texto) ? $creadores_texto : "Sin Asignar";
    ...
    case "guardaryeditar":
        ...
        $creador_per_ids = isset($_POST['creador_per_ids']) ? $_POST['creador_per_ids'] : [];
        $asignado_car_ids = isset($_POST['asignado_car_ids']) ? $_POST['asignado_car_ids'] : [];

        if (is_null($regla_id)) {
            // Llama a la nueva función de inserción con arrays
            $flujo_mapeo->insert_flujo_mapeo($cats_id, $creador_car_ids, $asignado_car_ids, $creador_per_ids);
        } else {
            // Llama a la nueva función de actualización con arrays
            $flujo_mapeo->update_flujo_mapeo($regla_id, $cats_id, $creador_car_ids, $asignado_car_ids, $creador_per_ids);
        }
        break;
